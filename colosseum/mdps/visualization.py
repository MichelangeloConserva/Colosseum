...
def plot_MDP_graph(
    ...
    node_labels: Union[bool, Dict["NodeType", Union[float, str]]] = None,
    action_labels: Union[bool, Dict[Tuple["NodeType", int], Union[float, str]]] = None,
    ...
    if mdp.is_episodic() and not continuous_form:
        if node_labels is not None and cm_state_labels is not None:
            node_color = [
                ...
                for node in mdp.episodic_graph().nodes
            ]
        else:
            node_color = [
                node_palette[5]  # brown
                if node[1] in mdp.starting_node_sampler.next_states and node[0] == 0
        ...
        if node_labels is not None and cm_state_labels is not None:
            ...
            node_color = [
                node_palette[5]  # brown
                if node in mdp.starting_node_sampler.next_states
                else node_palette[2]  # green
    ...
    if cm_state_labels is None:
        ...
        if mdp.communication_type == MDPClass.WEAKLY_COMMUNICATING:
...
def plot_MCGraph(
    ...
    labels: Union[bool, Dict["NodeType", Union[float, str]]] = {},
    font_color_labels="k",
    save_file: str = None,
    ax=None,
    figsize: Tuple[int, int] = None,
