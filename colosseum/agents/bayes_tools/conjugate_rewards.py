...
class N_NIG(ConjugateModel):
    ...
    def sample_MDP(self) -> np.ndarray:
        ...
        mean = self._rng.normal(loc=mu, scale=np.sqrt(var)).astype(np.float32)

        return mean.reshape(self.num_states, self.num_actions)
...
class N_N(ConjugateModel):
    ...
    def sample_MDP(self) -> np.ndarray:
        return self._rng.normal(
            loc=self.hyper_params[:, :, 0], scale=self.hyper_params[:, :, 1]
        ).astype(np.float32)
