...
@gin.configurable
class DQNEpisodic(NonTabularBsuiteAgentWrapper):
    """
    ...
    def produce_gin_file_from_parameters(parameters: Dict[str, Any], index: int = 0):
        ...
        for k, v in parameters.items():
            string += f"prms_{index}/DQNEpisodic.{k} = {v}\n"
        return string[:-1]
    ...
    def current_optimal_stochastic_policy(self) -> np.ndarray:
        ...
        qvals = (
            tf.stop_gradient(
                self._agent._forward(self.emission_map.all_observations.reshape(-1, d))
            )
            .numpy()
    ...
    def __init__(
        ...
        super(DQNEpisodic, self).__init__(seed, agent, mdp_specs)
