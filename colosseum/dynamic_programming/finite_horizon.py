...
from colosseum.dynamic_programming.utils import DynamicProgrammingMaxIterationExceeded
from colosseum.dynamic_programming.utils import argmax_3d
...
def episodic_value_iteration(
    ...
    Q = np.zeros((H + 1, n_states, n_actions), dtype=np.float32)
    V = np.zeros((H + 1, n_states), dtype=np.float32)
    for i in range(H):
        h = H - i - 1
...
def episodic_policy_iteration(T: np.ndarray, R: np.ndarray, gamma=0.99, epsilon=1e-7):
    ...
    Q = np.random.rand(H, n_states, n_actions)
    pi = argmax_3d(Q)
